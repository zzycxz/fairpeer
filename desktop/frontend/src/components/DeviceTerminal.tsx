...
export function DeviceTerminal({ device }: { device: string }) {
  ...
  return (
    <div className="termsession" aria-label={`人工终端 ${device}`}>
      <div className="termsession__bar">
        <span className="termsession__title">● REC {device} — 全程录制入审计（§6.1）</span>
        {err && <span className="termsession__err">{err}</span>}
        {exited && <span className="termsession__exited">会话已结束</span>}
        <span className="termsession__rec-note">{ready ? "在线" : "连接中…"}</span>
      </div>
