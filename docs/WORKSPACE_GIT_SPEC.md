...
> 完成；go 双模块 build/vet/test + 前端 tsc/vite/43 测试全绿；已提交（1ee52aaf，
> 混合文件按 hunk 手术剥离另一批次）。**2026-08-31 实测**：新构建运行实例上
> G1/G2/G3 全链路走通——改动页签显示分支 main + `+6 ?1`（与 git numstat 一致）、
> 非 git 仓库正确降级显示 gitUnavailable、分支切换 popover 正常弹出。
>
