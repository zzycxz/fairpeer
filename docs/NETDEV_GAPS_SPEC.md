...
| **G-P1** ✅v1 已修（J4/J5 拍板后开工：schema type/deny/policy/confirmers 软迁移+蓝队信封校验、SetActiveProject 后端态+切换审计、guardrail 域作用域 J4-A 只读+拒操作+deny 前缀、提案 Project 盖章（propose+runbook 两处）+ ExecuteProposal 跨域拒执、J5 ApproveProposalAs confirmers 名单校验+批准人入 Approver、前端切换器接线+三段式编辑器+批准人输入；v1.1 已修：allow=deny 的域内例外白名单（projectDenyVerdict，只豁免项目层）、policy=read-only 运行时写地板（命令层域内非 read 拒 + ExecuteProposal 整条拒含 cli 私有写路径）、trustdomain 升格接线（signApprovalWithDomain：启用且入域 → 操作者须对上本域身份显示名 + 域私钥 Ed25519 签名入 p.ApproverSig，启用未入域 fail-closed 拒批）） | **项目安全域**（PROJECT_SCENARIO_SPEC §二+§七 V1/V2） | 会话项目上下文管道（前端→后端会话态→guardrail/提案/发现）+ 提案补 project 字段 + NetDevProject 升级+ 三段式表单 | **~1.5-2 周（V1 上调）** |
...
| U3 ✅已裁决（2026-09-14）：J4=项目外设备只读+拒操作；J5=自报基线+trustdomain 开启时签名升格（折中）——G-P1 解除阻塞开工 | 裁决 J4/J5 | 原倾向即拍板结论，依据=NETDEV_PROJECT_DOMAIN_DESIGN §三 |
...
- **2026-09-14 追加批**（"尽量做"回合）：
  - **域闸 v1.2"工具族覆盖"核实为伪需求**——逐族勘察：docker（DockerGet GET 白名单）、firewall（GET-only、无 policy-write 路径）、dbquery（只读 allowlist）、netconf（显式拒 edit-config/copy-config/commit/lock 族）全部**结构性只读**，不存在未接线的设备写面；设备写路径仅 execSealed（已接域闸）与提案管线（已接盖章+跨域拒执+deny 覆盖）。
  - **remoteOnce 可用性修复**——SharedRemoteNode 失败不再被 sync.Once 钉死到进程结束（G-P1 批准升格抢先调用"未入域"错误会永久缓存）：改为锁内重试、成功后节点常驻；ResetSharedRemoteNode 同锁互斥。
  - **SeriesRead 读路径优化**（时间索引的安全替代）：①读不再持全局 seriesMu（写端 O_APPEND/原子替换语义下读者拿旧或新文件均合法，坏行有容错）——GpuBoard 批量构建不再堵死 RecordSeries；②mtime 短路——分片最后修改早于窗口起点直接空返回，闲置分片的 spark/board 读取 O(文件)→O(1)。
  - **F12 回归修复**（轮3测试补齐时抓到）：无驱动设备的 "no driver" problem 在并发组装时被 name 空检丢弃——problems 先于 name 检组装。
