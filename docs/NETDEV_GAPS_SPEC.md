...
| # | 缺口 | 现状/证据 | 修法方向 | 依赖 |
|---|---|---|---|---|
| **F1** | 部署 runbook 模板库（D-1 主体） | 部署蓝本在 MODEL_DEPLOY_SPEC §2.1，手工编排可用；模板机制（0.2.4 批次 B-D）已就绪但无部署模板 | 「vLLM 单机部署」「vLLM 多机」「昇腾 vllm-ascend」三条模板入库，参数 `{{tp}}/{{model_path}}/{{port}}` 由机型档案缺省填入 | E3/E4 |
