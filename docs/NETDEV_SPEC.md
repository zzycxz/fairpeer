...
> **✅ 已实现（2026-09-12，统一急停语义，`internal/netdev/estop.go`）**：急停 = 杀全部设备连接/人工终端/发现任务 + **running 割接置 Hold**（在步骤边界暂停——不是 abort：Abort 不回退已执行变更，会把设备留在半割接态；继续/回退/终止由人按）+ running Job 边界暂停（文案"紧急停止"，区别于人工暂停）+ executing 提案代际冻结（`estopGen` 代数，步骤边界检测即走既有 partial 冻结路径，未落盘步骤不执行）。全部入审计；枚举失败进 `EstopReport.Errors` 不静默；有割接被停时 IM 深链召回值班。
...
| 5 | **紧急停止 × 执行中提案**：停止即中断 executing，制造 partial | 中 | **✅ 已实现（2026-09-12）**：急停在提案步骤边界冻结（代数检测 → 既有 partial 路径，未落盘步骤不执行）；已落盘前缀保持原状走 B-2 partial 裁决（回滚或保留，均为人工动作）；割接侧同步置 Hold 引导决策 |
| 6 | **并发与 VTY 枯竭**：巡检与对话同时跑，老设备仅 5 VTY | 中 | `max_sessions_per_device`（默认 2）+ 每设备信号量；巡检任务与交互会话共享配额，超限排队 |
| 7 | **Telnet 凭证明文过网** | 中 | 已有 opt-in + 警告；补充：telnet 设备的凭证在 secret store 标记 `cleartext-risk`，审计记录telnet 会话的明文性质。**✅ 已裁决（2026-09）：telnet 传输已删除（config/netdev.go §6.4），本条关闭** |
