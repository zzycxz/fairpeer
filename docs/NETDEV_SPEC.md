...
> **版本**: v0.1 | **日期**: 2026-08-18 | **状态**: 待评审（含附录 B 评审发现，须先裁决）
>
...
| # | 问题 | 严重度 | 建议裁决 |
|---|------|--------|----------|
| 1 | **未知命令默认归类**：白名单不可穷举，unknown→write 会挡住合法诊断（新型号读命令） | 高 | 维持默认 write（保守正确），但提供「读命令模式」对话框：unknown 命令一键临时标记为 read（仅本会话生效、留审计），管理员可在知识库永久归类 |
...
| 9 | **弱口令核查与尝试预算矛盾**：3 次预算无法做字典核查 | 中 | 收窄为「默认口令/空口令/工号口令」小集合核查（预算内可完成）；字典级核查列为非目标 |
