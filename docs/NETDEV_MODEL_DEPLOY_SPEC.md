...
| L-变体 | 单机 / 多机(head-worker 序) / **版本升级蓝绿** / air-gap / **DP 扩副本**（不停机可自动）/ **TP 重排=蓝绿换队**（TP 启动固定，改必重启）/ **故障节点替换**（ECC 阈值分档：>10 次/时 correctable→drain、反复 uncorrectable→cordon、换卡人工）/ **服务下线**（idle→摘流→删 endpoint→归档） | 7-8 | **流量编排原语统一金丝雀/灰度/回滚**（双版本+流量百分比，KServe 三角色状态机）；双版本显存翻倍→授权人工 |
