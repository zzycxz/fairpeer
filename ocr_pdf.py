...
def _ocr_page(pdf_path: str, page_idx: int, ocr_engine) -> str:
    ...
    tmp.close()
    # The temp PNG is removed on EVERY exit path — a render failure used to
    # leak one PNG per broken page into the temp directory.
    try:
        doc = None
        try:
...
def extract_pdf(pdf_path: str, lang: str = "ch", first: int = 1, last: int = 0) -> tuple:
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for offset, page in enumerate(page_slice):
            ...
            # Per-page fault isolation: one corrupt content stream must cost
            # its own page only, never the whole batch (R-1).
            try:
                # Tables.
                ...
                WARNINGS.append(f"page {i + 1}: text/table extraction failed ({e}); content may be missing")
                continue
    ...
    # OCR scanned pages.
    if scanned_indices:
        # Dependency problems (install failure, no network, broken paddleocr
        # version) are confined to the OCR pages — text pages already in
        # all_parts must still be returned (R-4).
        try:
            _ensure_ocr()
            _ensure_fitz()
            ...
            WARNINGS.append(f"pages {[i + 1 for i in scanned_indices]}: OCR unavailable ({e}); content may be missing")
        else:
