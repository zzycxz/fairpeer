...
**Q：为什么有时候连不上？**
A：linkpeer 采用纯 P2P，不设中继服务器。当手机和桌面**双方都处在对称 NAT**（中国移动 4G / 运营商 CGNAT 家宽常见）时，NAT 物理上打不通。解决办法：① 两端接同一 WiFi（必连）；② 给桌面端配公网 IP 或开启路由器 UPnP / 端口转发；③ 手机切换网络。这是「绝不过云」这一选择的代价——我们选择不为了兜底而设一台能解密你流量的中继。
